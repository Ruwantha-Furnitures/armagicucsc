...
function CustomerCheckoutDeteailsForm() {
  ...
  const getAllDistricts = async () => {
    try {
      ...
      let res = await axios.get("/api/deliveryCharge/");
  ...
  const getProductData = async (id) => {
    try {
      const res = await axios.get(`/api/api/products/${id}`); // wil receive the response
  ...
  const redirectPaymentPage = (
    <Redirect to="/customer_productDetails_payment" />
  );
  return (
    <React.Fragment>
                  ...
                  <Form style={{ padding: "20px" }} onSubmit={submitHandler}>
                    ...
                    >
                      ...
                      {district.map((districtList) => (
                        <option value={districtList.area}>
                          {districtList.area}
                        </option>
                      ))}
                    </select>
                      ...
                      <Col>
                        <Form.Label>
                          <i style={{ fontSize: "9px" }}>
                            **Delivery charged will be added. Click on Continue
                            for payment
                          </i>
                        </Form.Label>
                      </Col>
