...
function CustomerProductDetails() {
  ...
  const setCartValue = async (itemid) => {
    ...
    try {
      ...
      const CustomerInCartRespond = await axios.get(
        `/api/customerCart/customer_id/${customer_id}/product_id/${product_id}`
      );
      ...
      if (CustomerInCartRespond.data) {
        ...
        const updaterespond = await axios.put(`/api/cart/${cartID}`, {
        ...
        const respond = await axios.post("/api/cart", data);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const res = await axios.get(`/api/product/${itemID}`); // wil receive the response
  ...
  const getRate = async () => {
    var sellProducts = [];
    const product_id = localStorage.getItem("productID");
    const setRateDetailsResponse = await axios.get(
      `/api/productReviewForAProduct/${product_id}`
    );
                ...
                <Col sm={6}>
                  <Container>
                    <center>
                      ...
                      <button
                        onClick={() => setCartValue(productDetails.id)}
                        class="addtocart"
                      >
                        Add to cart{" "}
                      </button>{" "}
