...
function PasswordRecoveryForm() {
  ...
  const submitHandler = async (e) => {
    ...
    try {
      const TokenResponse = await axios.get(`/api/resetTokenByToken/${token}`);
      ...
      if (newpassword === confirmpassword) {
        ...
        const NewPasswordResponse = await axios.put(
          `http://localhost:8080/api/accountForCustomer/${TokenResponse.data.email}`,
          NewPasswordData
        );
        ...
        if (NewPasswordResponse.status === 200) {
          ...
          const tokenDeleteResponse = axios.delete(
            `http://localhost:8080/api/resetTokenByToken/${token}`
          );
        ...
        <div>
          ...
          >
            <Form style={{ padding: "15px" }} onSubmit={submitHandler}>
              <Row style={rowStyle}>
                <center>
                  <img src={Avatar} alt={Avatar} width={40} height={40}></img>
                </center>
                <center>
                  <h4 style={title}>Change Password</h4>
                </center>
              </Row>
