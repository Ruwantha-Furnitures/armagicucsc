...
function ThankYouPage() {
  ...
  useEffect(async () => {
    const orders = {
      total_product_amount: afterDiscount,
      customer_id: customer_id,
      payment_method: payment_method,
    };
    console.log(orders);
    try {
      let response = await axios.post("/api/order", orders);
    ...
    //get order ID
    try {
      const maxOrderIDResponse = await axios.get("/api/order");
    ...
    try {
      let response = await axios.post("/api/payment", paymentdata);
      console.log(response.data);
      if (response.status === 200) {
    ...
    try {
      let response = await axios.post("/api/shippingDetail", shippingdata);
      console.log(response.data);
      if (response.status === 200) {
    ...
    const cartResponse = await axios.get(
      `/api/customerCart/customer_id/${customer_id}`
    );
    ...
    for (let i = 0; i < Number(length); i++) {
      ...
      try {
        let response = await axios.post("/api/sellProduct", sellProductdata);
        console.log(response.data);
        if (response.status === 200) {
      ...
      console.log(cartID);
      try {
        let response = await axios.delete(`/api/customerCart/id/${cartID}`);
        console.log(response.data);
      } catch (error) {
        console.log(error);
      }
    ...
    // const productResponse = await axios.get(`/api/product/${product_id}`);
    ...
    // try{
    //     let response = await axios.post('h/api/sellProduct',sellProductdata);
    //     console.log(response.data);
    //     if(response.status === 200){
    //       setIsSellProductSubmit(true)
    //     }else{
    ...
    //       let response = await axios.delete(`h/api/customerCart/id/${id}`)
    //       console.log(response.data);
    //       if(response.status === 200){
  ...
  return (
    <React.Fragment>
      {isOrderSubmit === true &&
        ...
        isSellProductSubmit === true &&
        isShippingDataSubmit === true &&
        redirectThankYou}
      {IsCartDeleted === false && (
