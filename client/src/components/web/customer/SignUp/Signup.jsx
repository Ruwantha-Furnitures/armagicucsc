...
const Signup = () => {
  ...
  const signUpHandler = async (data) => {
    ...
    try {
      ...
      // create account
      const responseAccount = await axios.post("/api/account", account);
      ...
      // customer create
      const responseCustomer = await axios.post("/api/customer", customer);
      ...
      console.log(
        responseCustomer.data +
          ...
          " " +
          responseOnlineCustomer.data
      );

      if (
        responseCustomer.status === 200 &&
        responseAccount.status === 200 &&
        responseOnlineCustomer.status === 200
      ) {
